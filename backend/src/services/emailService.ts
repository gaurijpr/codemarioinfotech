...
export class EmailService {
  ...
  private initTransporter() {
    ...
    if (host && user && pass) {
      ...
      this.isConfigured = false;
      console.log(
        '[EmailService] Notice: SMTP credentials are not fully configured in .env. Operating in Simulation / Safe-Log mode. Inquiries will be logged to server console.'
      );
    }
  }

  public getStatus(): { isConfigured: boolean; recipient: string } {
    return {
  ...
  public async sendInquiry(data: InquiryData): Promise<EmailResult> {
